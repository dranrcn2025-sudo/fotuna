...
const collectAllLinkableTitles = (books) => {
  const m = new Map();
  const c = (es, bid, bt) => es.forEach(e => {
    if (e.linkable) { if (!m.has(e.title)) m.set(e.title, []); m.get(e.title).push({ bookId: bid, bookTitle: bt, entry: e }); }
    if (e.children?.length) c(e.children, bid, bt);
  });
  books.forEach(b => c(b.entries, b.id, b.title));
  return m;
};

const findEntryPath = (es, tid, p = []) => {
  for (const e of es) {
    const cp = [...p, e];
    if (e.id === tid) return cp;
    if (e.children?.length) { const f = findEntryPath(e.children, tid, cp); if (f) return f; }
  }
  return null;
};

const findEntryById = (es, id) => {
  for (const e of es) {
    if (e.id === id) return e;
    if (e.children?.length) { const f = findEntryById(e.children, id); if (f) return f; }
  }
  return null;
};
...
const getAllChildContent = (entry, allEntries) => {
  ...
  const collect = (e) => {
    ...
    if (e.children?.length) e.children.forEach(ch => collect(findEntryById(allEntries, ch.id) || ch));
  };
  if (entry?.children?.length) entry.children.forEach(ch => collect(findEntryById(allEntries, ch.id) || ch));
  return contents;
};
...
// 富文本编辑器
const RichEditor = React.forwardRef(({ content, onSave, fontFamily, activeFormats }, ref) => {
    ...
    if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
      ...
      if (hasF) {
        ...
        if (sel.rangeCount > 0) {
          ...
          else if (activeFormats.size === 'big') document.execCommand('fontSize', false, '5');
          else if (activeFormats.size === 'huge') document.execCommand('fontSize', false, '6');
          sel.collapseToEnd();
        }
        triggerSave();
      }
  ...
  React.useImperativeHandle(ref, () => ({
    forceSave,
    execCommand: (cmd, val) => { if (editorRef.current) { editorRef.current.focus(); document.execCommand(cmd, false, val); triggerSave(); } },
    insertHtml: (html) => { if (editorRef.current) { editorRef.current.focus(); document.execCommand('insertHTML', false, html); triggerSave(); } }
  }));
  
  useEffect(() => () => { if (saveTimer.current) clearTimeout(saveTimer.current); }, []);
  
  return <div ref={editorRef} className="rich-editor" contentEditable onInput={triggerSave} onPaste={(e) => { e.preventDefault(); document.execCommand('insertText', false, e.clipboardData.getData('text/plain')); triggerSave(); }} onKeyDown={handleKeyDown} onBlur={forceSave} style={{ fontFamily }} suppressContentEditableWarning />;
});
  ...
  return (
    <div className="sidebar-item-wrapper">
      <div className={'sidebar-item ' + (currentId === entry.id ? 'active' : '')} style={{ paddingLeft: (12 + depth * 16) + 'px' }} onClick={() => onSelect(entry)}>
        {hasC && <span className={'expand-icon ' + (isExp ? 'expanded' : '')} onClick={(e) => { e.stopPropagation(); onToggle(entry.id); }}>›</span>}
      ...
      {hasC && isExp && entry.children.map(c => <SidebarItem key={c.id} entry={c} depth={depth + 1} onSelect={onSelect} currentId={currentId} expandedIds={expandedIds} onToggle={onToggle} />)}
    </div>
  );
};
...
const ContextMenu = ({ isOpen, position, onClose, options }) => isOpen ? (
  <>
    ...
    <div className="context-menu" style={{ top: position.y, left: Math.min(position.x, window.innerWidth - 180) }}>
      {options.map((o, i) => <div key={i} className={'context-item ' + (o.danger ? 'danger' : '')} onClick={() => { o.action(); onClose(); }}><span className="context-icon">{o.icon}</span>{o.label}</div>)}
    </div>
  </>
) : null;
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content book-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="cover-section">
          ...
          <button className="upload-cover-btn" onClick={() => fileRef.current?.click()}>📷 上传封面</button>
          <input ref={fileRef} type="file" accept="image/*" onChange={async e => { const f = e.target.files[0]; if (f) setCoverImage(await compressImage(f, 400)); }} style={{ display: 'none' }} />
        </div>
        <div className="modal-actions">
          <button className="btn-cancel" onClick={onClose}>取消</button>
...
// 格式菜单
const TextFormatMenu = ({ isOpen, onClose, activeFormats, onToggleFormat }) => isOpen ? (
  <>
    <div className="format-menu-overlay" onClick={onClose} />
    <div className="format-menu">
      <p className="format-hint">点亮后输入即带格式</p>
      <div className="format-row">
        <button className={activeFormats.bold ? 'active' : ''} onClick={() => onToggleFormat('bold')}><b>B</b></button>
        <button className={activeFormats.italic ? 'active' : ''} onClick={() => onToggleFormat('italic')}><i>I</i></button>
        <button className={activeFormats.underline ? 'active' : ''} onClick={() => onToggleFormat('underline')}><u>U</u></button>
        <button className={activeFormats.strike ? 'active' : ''} onClick={() => onToggleFormat('strike')}><s>S</s></button>
      </div>
      <div className="format-row size-row">
        <button className={activeFormats.size === 'small' ? 'active' : ''} onClick={() => onToggleFormat('small')}>小</button>
        <button className={activeFormats.size === 'medium' ? 'active' : ''} onClick={() => onToggleFormat('medium')}>中</button>
        <button className={activeFormats.size === 'big' ? 'active' : ''} onClick={() => onToggleFormat('big')}>大</button>
        <button className={activeFormats.size === 'huge' ? 'active' : ''} onClick={() => onToggleFormat('huge')}>特大</button>
      </div>
    </div>
  </>
) : null;

const AlignMenu = ({ isOpen, onClose, onAlign }) => isOpen ? (
  <>
    <div className="format-menu-overlay" onClick={onClose} />
    <div className="format-menu align-menu">
      <div className="format-row">
        <button onClick={() => { onAlign('justifyLeft'); onClose(); }}><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3 3h18v2H3V3zm0 4h12v2H3V7zm0 4h18v2H3v-2zm0 4h12v2H3v-2zm0 4h18v2H3v-2z"/></svg></button>
        <button onClick={() => { onAlign('justifyCenter'); onClose(); }}><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3 3h18v2H3V3zm3 4h12v2H6V7zm-3 4h18v2H3v-2zm3 4h12v2H6v-2zm-3 4h18v2H3v-2z"/></svg></button>
        <button onClick={() => { onAlign('justifyRight'); onClose(); }}><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3 3h18v2H3V3zm6 4h12v2H9V7zm-6 4h18v2H3v-2zm6 4h12v2H9v-2zm-6 4h18v2H3v-2z"/></svg></button>
      </div>
    </div>
  </>
) : null;
...
const FontMenu = ({ isOpen, onClose, onSelectFont, currentFont }) => {
  ...
  return isOpen ? (
    <>
      <div className="format-menu-overlay" onClick={onClose} />
      <div className="font-menu">{fonts.map(f => <div key={f.v} className={'font-item ' + (currentFont === f.v ? 'active' : '')} onClick={() => { onSelectFont(f.v); onClose(); }} style={{ fontFamily: f.v }}>{f.n}</div>)}</div>
    </>
  ) : null;
};

// 工具栏
const EditorToolbar = ({ onIndent, onFormat, onAlign, onFont, onImage, hasActive }) => {
  const imgRef = useRef(null);
  return (
    <div className="editor-toolbar-bottom">
      <button onClick={onIndent} title="全文缩进">↵</button>
      <button onClick={onFormat} className={hasActive ? 'has-active' : ''} title="文字格式">A</button>
      <button onClick={onAlign} title="对齐"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M3 3h18v2H3V3zm3 4h12v2H6V7zm-3 4h18v2H3v-2zm3 4h12v2H6v-2zm-3 4h18v2H3v-2z"/></svg></button>
...
// FAB菜单
const AddMenu = ({ isOpen, onClose, onAddEntry, onAddFolder, onReorder }) => isOpen ? (
  <>
    <div className="add-menu-overlay" onClick={onClose} />
    <div className="add-menu">
      <div className="add-menu-item" onClick={() => { onReorder(); onClose(); }}><span>↕️</span><span>调整排序</span></div>
...
const ReorderList = ({ entries, onReorder, onExit }) => {
  ...
  return (
    <div className="reorder-mode">
      ...
      <div className="reorder-list" ref={ref} onTouchMove={(e) => { if (di === null) return; e.preventDefault(); const t = e.touches[0]; const items = ref.current?.querySelectorAll('.reorder-item'); if (items) for (let i = 0; i < items.length; i++) { const r = items[i].getBoundingClientRect(); if (t.clientY >= r.top && t.clientY <= r.bottom) { setOi(i); break; } } }} onTouchEnd={() => { if (di !== null && oi !== null && di !== oi) onReorder(di, oi); setDi(null); setOi(null); }}>
        {entries.map((e, i) => (
          <div key={e.id} className={'reorder-item ' + (di === i ? 'dragging' : '') + ' ' + (oi === i && di !== i ? 'over' : '')} onTouchStart={() => { setDi(i); if (navigator.vibrate) navigator.vibrate(30); }}>
            <div className="reorder-content"><span className="reorder-icon">{e.isFolder ? '📁' : '📄'}</span><span className="reorder-title">{e.title}</span></div>
...
export default function App() {
  ...
  const initMerged = useCallback((e) => {
    if (!e || !currentBook) return;
    setMergedContents(getAllChildContent(e, currentBook.entries).map(i => ({ id: i.id, title: i.title, content: i.content || '', isNew: false })));
  }, [currentBook]);
  ...
  const handleBackToShelf = () => { if (editorRef.current) editorRef.current.forceSave?.(); setSlideAnim('slide-out'); setTimeout(() => { setCurrentBook(null); setCurrentEntry(null); setViewMode('list'); setNavigationStack([]); setIsSidebarOpen(false); setJumpHistory([]); setIsReorderMode(false); setSlideAnim(''); }, 200); };
  const handleEntryClick = (entry) => { if (editorRef.current) editorRef.current.forceSave?.(); setSlideAnim('slide-in'); setNavigationStack(prev => [...prev, currentEntry].filter(Boolean)); setCurrentEntry(entry); if (entry.isFolder || entry.children?.length > 0) setViewMode('list'); else { setViewMode('single'); setIsReadOnly(true); } setTimeout(() => setSlideAnim(''), 250); };
  const handleBack = () => { if (editorRef.current) editorRef.current.forceSave?.(); setSlideAnim('slide-out'); setTimeout(() => { if (navigationStack.length > 0) { const p = navigationStack[navigationStack.length - 1]; setNavigationStack(s => s.slice(0, -1)); setCurrentEntry(p); setViewMode('list'); } else { setCurrentEntry(null); setViewMode('list'); } setSlideAnim(''); setIsReorderMode(false); }, 200); };
  ...
  const handleSidebarSelect = (entry) => { if (editorRef.current) editorRef.current.forceSave?.(); const p = findEntryPath(currentBook.entries, entry.id); if (p) { setNavigationStack(p.slice(0, -1)); setCurrentEntry(entry); if (entry.isFolder || entry.children?.length > 0) setViewMode('list'); else setViewMode('single'); } setIsSidebarOpen(false); };

  const handleLinkClick = useCallback((kw, tbid, teid) => {
    if (editorRef.current) editorRef.current.forceSave?.();
    setJumpHistory(p => [...p, { bookId: currentBook.id, entry: currentEntry, navStack: navigationStack, viewMode }]);
    const tb = data.books.find(b => b.id === tbid);
    if (tb) {
      setSlideAnim('slide-in'); setCurrentBook(tb);
      const path = findEntryPath(tb.entries, teid);
      if (path) { const te = path[path.length - 1]; setNavigationStack(path.slice(0, -1)); setCurrentEntry(te); if (te.isFolder && te.linkable) { setViewMode('merged'); setTimeout(() => initMerged(te), 0); } else if (te.isFolder) setViewMode('list'); else setViewMode('single'); }
      setTimeout(() => setSlideAnim(''), 250);
    }
  }, [currentBook, currentEntry, navigationStack, viewMode, data.books, initMerged]);

  const handleMergedChange = (i, f, v) => {
    const nc = mergedContents.map((x, j) => j === i ? { ...x, [f]: v } : x);
    setMergedContents(nc);
    const item = nc[i];
    if (f === 'content') saveContent(v, item.id, currentBook.id);
    else if (f === 'title') setData(prev => ({ ...prev, books: prev.books.map(b => b.id === currentBook.id ? { ...b, entries: updateEntryInTree(b.entries, item.id, { title: v }) } : b) }));
  };

  const handleAddMerged = () => {
    const newId = generateId();
    const newEntry = { id: newId, title: '新词条', summary: '', content: '', isFolder: false, linkable: true, children: [] };
    setData(prev => ({ ...prev, books: prev.books.map(b => b.id === currentBook.id ? { ...b, entries: addEntryToParent(b.entries, currentEntry.id, newEntry) } : b) }));
    setMergedContents(prev => [...prev, { id: newId, title: '新词条', content: '', isNew: false }]);
  };
  ...
  const handleAlign = (c) => { if (editorRef.current) editorRef.current.execCommand(c, null); };
  const handleIndent = () => { const ed = document.querySelector('.rich-editor'); if (!ed) return; ed.querySelectorAll('p').forEach(p => { if (p.textContent && !p.textContent.startsWith('　　')) p.textContent = '　　' + p.textContent; }); if (editorRef.current) editorRef.current.forceSave?.(); };
  const handleImageUpload = async (e) => { const f = e.target.files[0]; if (f && editorRef.current) { const c = await compressImage(f, 600); editorRef.current.insertHtml('<p style="text-align:center"><img src="' + c + '" style="max-width:100%;border-radius:8px" /></p>'); } e.target.value = ''; };
  const handleEntrySwipe = (entry, dx) => { if (dx < -80 && (entry.isFolder || entry.children?.length > 0)) { if (editorRef.current) editorRef.current.forceSave?.(); setSlideAnim('slide-in'); setNavigationStack(p => [...p, currentEntry].filter(Boolean)); setCurrentEntry(entry); setViewMode('merged'); setTimeout(() => initMerged(entry), 50); setTimeout(() => setSlideAnim(''), 250); } };
  ...
  if (!currentBook) return (
    <div className="app bookshelf-view">
      ...
      <div className="bookshelf">
        {data.books.map(b => (
          <div key={b.id} className="book-card" style={{ '--book-color': b.color }} onClick={() => handleBookSelect(b)} onTouchStart={e => handleLongPressStart(e, 'book', b)} onTouchEnd={handleLongPressEnd} onTouchMove={handleLongPressEnd}>
            <div className="book-spine"></div>
            <div className="book-cover">{b.coverImage ? <img src={b.coverImage} alt="" className="cover-image" /> : <span className="book-emoji">{b.cover}</span>}</div>
            <div className="book-shadow"></div>
            <div className="book-meta"><h2>{b.title}</h2>{b.author && <p>{b.author} 著</p>}</div>
          </div>
        ))}
  ...
  return (
    <div className="app main-view">
      ...
      <div className="main-content" onTouchStart={e => { touchStartX.current = e.touches[0].clientX; }} onTouchEnd={e => { if (e.changedTouches[0].clientX - touchStartX.current > 80 && (currentEntry || navigationStack.length > 0)) handleBack(); }}>
        ...
        <main className={'content-area ' + slideAnim}>
          {viewMode === 'list' && !isReorderMode && (<>
            ...
            <div className="entry-list">{currentEntries.map(entry => { let tx = 0; return (<div key={entry.id} className="entry-card" onClick={() => handleEntryClick(entry)} onTouchStart={e => { tx = e.touches[0].clientX; handleLongPressStart(e, 'entry', entry); }} onTouchMove={handleLongPressEnd} onTouchEnd={e => { handleLongPressEnd(); handleEntrySwipe(entry, e.changedTouches[0].clientX - tx); }}><div className="entry-icon">{entry.isFolder ? '📁' : '📄'}</div><div className="entry-info"><h3>{entry.title}{entry.linkable && <span className="star-badge">⭐</span>}</h3><p>{entry.summary}</p></div><span className="entry-arrow">›</span></div>); })}</div>
        ...
        {isEditing && <EditorToolbar onIndent={handleIndent} onFormat={() => setShowFormatMenu(true)} onAlign={() => setShowAlignMenu(true)} onFont={() => setShowFontMenu(true)} onImage={handleImageUpload} hasActive={hasActiveFormat} />}
...
.reorder-title{font-size:1rem;color:#2D3047}
.bookmark-tab{width:40px;min-height:52px;background:linear-gradient(135deg,#8B7355,#6B5335);display:flex;align-items:center;justify-content:center;color:#f4e4c1;font-size:1.2rem}
